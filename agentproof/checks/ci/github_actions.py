...
def detect_ci_integrity(diff: dict[str, dict[str, list[tuple[int, str]]]]) -> list[Finding]:
    ...
    for path, chunks in diff.items():
        if not (".github/workflows/" in path or path.lower() in {"tox.ini", "pytest.ini", "pyproject.toml", "package.json", "makefile", "cargo.toml", "go.mod"}):
            continue
        ...
        if any(re.search(r"(?:pytest|jest|vitest|npm\s+(?:run\s+)?test|cargo\s+test|go\s+test)", text, re.IGNORECASE) for text in added + removed):
