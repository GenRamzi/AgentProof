...
def detect_dependency_integrity(changed_files: list[str], diff: dict[str, dict[str, list[tuple[int, str]]]]) -> list[Finding]:
    ...
    for path, chunks in diff.items():
        ...
        dependency_lines = [text for text in added if re.search(r"(?:dependencies|devDependencies|requires|crate|require\s+\w+)", text, re.IGNORECASE)]
        if len(dependency_lines) >= 3:
